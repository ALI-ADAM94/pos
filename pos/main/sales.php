...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include'../connect.php';
				$result = $db->query("SELECT * FROM sales_order WHERE invoice ='".$id."' ");
			...
			<tr>
				...
				$sdsd=$_GET['invoice'];
				$resultas = $db->query("SELECT sum(amount) FROM sales_order WHERE invoice= '".$sdsd."'");
				//$resultas->bindParam(':a', $sdsd);
				//$resultas->execute();
				for($i=0; $rowas = $resultas->fetch_array(); $i++){
				...
				<td colspan="1"><strong style="font-size: 12px; color: #222222;">
			<?php 
				$resulta = $db->query("SELECT sum(profit) FROM sales_order WHERE invoice= '".$sdsd."'");
				//$resulta->bindParam(':b', $sdsd);
				//$resulta->execute();
				for($i=0; $qwe = $resulta->fetch_array(); $i++){
				$asd=$qwe['sum(profit)'];
